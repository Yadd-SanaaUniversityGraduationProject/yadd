...
حتى يحدد الفريق أفضلية كل عضو لاحقًا، تستخدم الرموز `Student A`, `Student B`, `Student C`. المسؤولية تتغير بين المراحل لمنع اعتماد المشروع على شخص واحد.

### المرحلة 1 — تثبيت الأساس والبحث والمتطلبات
**الفترة: 1–3 سبتمبر**

| المسؤول | المخرجات المطلوبة |
|---|---|
| Student A | مراجعة Chapter 1 foundation: Baseline/Scope/Objectives/Methodology + Work Plan + Risks + Feasibility gaps. |
| Student B | Chapter 2 research: حِرفة، أشغال، الأنظمة السابقة، Source Register، Comparison Matrix، Gap Statement. |
| Student C | Chapter 3 foundation: Stakeholders/Actors + Data Gathering (SUR-01 only) + SRS blockers + Business Rules/Lifecycles الأساسية. |
...
| المسؤول | المخرجات المطلوبة |
|---|---|
| Student A | Use Case Diagram + أهم Use Case Specifications + Activity Diagrams للعمليات الحرجة. |
| Student B | DFD/Process/Data Flow/Data Store descriptions **وفق نتيجة GOV-Q02**؛ إذا لم يحسم القرار، يعد مسودة منفصلة موسومة بأنها قيد التوافق المنهجي ولا تدمج منطقيًا مع UML على أنها منهج واحد. |
| Student C | Sequence Diagrams الأساسية + Class/Domain model + بدء ERD من SRS/Business Rules المستقرة. |
...
| المسؤول | المخرجات المطلوبة |
|---|---|
| Student A | Database Design: ERD review → Relation Schema → Data Dictionary الأساسي. |
| Student B | Interface Design: hierarchy + wireframes للشاشات الأساسية + Forms/Queries/Reports المطلوبة أكاديميًا. |
| Student C | Feasibility (Economic/Technical/Operational) + NFR review + Architecture/AI boundaries + التأكد من قابلية التصميم للتنفيذ لفريق ثلاثة طلاب. |
...
| المسؤول | المخرج |
|---|---|
| Student A | تجميع وتنقيح Chapter 1 من مصادر الحقيقة الحالية. |
| Student B | تجميع وتنقيح Chapter 2، خصوصًا حِرفة/أشغال والفجوة الجديدة. |
| Student C | تجميع Chapter 3. |
| الثلاثة | Chapter 4 يقسم: A قاعدة البيانات، B الواجهات، C الدمج الهندسي والاتساق. |
...
تقسم المراجعة Cross-Review بحيث لا يراجع الطالب عمله فقط:

- Student A يراجع Chapter 2 + Chapter 3 models.
- Student B يراجع Chapter 1 + Database/ERD.
- Student C يراجع Chapter 4 interfaces + SRS/Traceability.
