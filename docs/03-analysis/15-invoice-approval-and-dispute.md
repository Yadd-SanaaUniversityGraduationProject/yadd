...
- يرسل النظام Reminder بعد 24 ساعة، وReminder ثانيًا بعد 48 ساعة، وبعد 72 ساعة تصبح الفاتورة `Pending Customer Approval — Overdue`.
- لا يوجد `Auto-Approval` مهما طال الانتظار.
...
- `INV-BR-14`: طلب Revision يتطلب ملاحظة ويحفظ Version History؛ لا حد عددي صلب، وبعد ثاني Revision متتالٍ يظهر Complaint prompt.
- `INV-BR-15`: Complaint يتطلب سببًا ووصفًا إلزاميين ومرفقات اختيارية؛ الحل يعيد المسار للمراجعة/التعديل وعدم الحل ينتهي Disputed.

## 9. سياسة عدم الاستجابة المعتمدة

- 24h: Reminder #1.
- 48h: Reminder #2.
- 72h: `Pending Customer Approval — Overdue`.
- لا Auto-Approval ولا Completed بدون اعتماد Beneficiary.