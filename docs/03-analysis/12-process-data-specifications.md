...
- يمكن للـ`Provider` تعديل الاستجابة أو سحبها ما دام `Request` بحالة `Open` ولم يتم اختيار مقدم؛ ولا توجد Provider Response TTL مستقلة، إذ تنتهي فعاليتها مع Withdraw/Selection/Request Close/Expiry — DEC-082.
...
- Complaint قبل الاعتماد تتطلب reason + description ومرفقات اختيارية؛ إذا استمر النزاع دون حل، تصبح `Transaction = Disputed`، وهي حالة نهائية غير ناجحة — `DEC-073/084`.
...
- تقييم `Beneficiary→Provider` مطلوب بعد `Completed` بنموذج Hybrid: Overall 1–5 + خمسة Structured Textual Criteria حسب Provider Type + تعليق اختياري. يمكن اختيار Later، مع Reminder بعد 24h، ويلزم إكمال التقييم السابق قبل Transaction جديدة — DEC-087.
...
- In-App Notification هي القناة الافتراضية؛ SMS للأمان/OTP والحساب الحرِج، والبريد الموثق اختياري — DEC-090.
