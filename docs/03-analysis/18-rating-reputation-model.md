...
> **الحالة:** `ANALYZED_APPROVED — SYNCHRONIZED 2026-09-18 THROUGH DEC-087`
>
> **القرارات المرجعية:** DEC-051/052/063/071/087.
...
بعد اعتماد الفاتورة:
- يصبح تقييم مقدم الخدمة/المنتج التزامًا Post-Transaction على المستفيد، مع إمكانية اختيار Later.
- يرسل النظام Reminder بعد 24 ساعة إذا لم يقيّم.
- قبل بدء Transaction جديدة، يجب استكمال أي تقييم Provider سابق مطلوب.
- التقييم Hybrid: Overall Stars 1–5 + Structured Textual Criteria + Optional Comment.
- Service Provider criteria: جودة الخدمة، الالتزام بالموعد، الالتزام بالاتفاق، التواصل والاستجابة، التعامل والاحترافية.
- Product Provider criteria: مطابقة المنتج للوصف، جودة المنتج، الالتزام بالتجهيز/الموعد، التواصل والاستجابة، الالتزام بالاتفاق.
- يظهر للعامة الاسم الأول فقط للمقيّم مع دلالة `Verified Transaction Review`/تعامل مكتمل، دون بيانات شخصية أخرى.
...
- `RAT-BR-03`: Overall Stars 1–5 جزء إلزامي من نموذج Hybrid، مع Structured Textual Criteria وتعليق اختياري — `APPROVED` وفق DEC-087.
...
- `RAT-BR-09`: يسمح Later ثم Reminder 24h، ويمنع بدء Transaction جديدة قبل إكمال تقييم Provider السابق المطلوب — `APPROVED`.
- `RAT-BR-10`: الاسم العام للمقيّم = First Name فقط مع Verified Transaction Review indicator — `APPROVED`.
